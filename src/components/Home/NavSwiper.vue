      ...
      <div class="navigation">
        <ul>
          ...
          >
            <router-link to="/">
              {{ item.categoryName }}
              <el-icon color="#ffffff" :size="16"><ArrowRight /></el-icon>
            </router-link>
...
import { getFirstCategorys } from '../../api/courseClassify';
import { tagsList, searchCourse } from '../../api/courseManagement';
import { getSliders } from '../../api/carousel';
import { ref, onBeforeMount } from 'vue';
import { courseType } from '../../utils/courseType';
export default {
  setup() {
    //一级课程分类的数据
    let FirstListRef = ref([]);
    //鼠标移入的默认布尔值
    ...
    onBeforeMount(() => {
      ...
      getFirstCategorys().then((res) => {
        FirstListRef.value = res.data.list;
      });
      //获取图片轮播数据
      getSliders().then((res) => {
...
.navigation ul li:hover .category-detail {
  display: block;
}
