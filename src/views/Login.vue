...
import { loginByJson, sendCaptcha, loginByMobile } from '../api/user';
  ...
  setup() {
    ...
    let rules = reactive({
      username: [
        ...
        { min: 3, max: 12, message: '请输入3-11位用户名', trigger: 'blur' }
      ],
      userpwd: [
        { required: true, message: '请输入密码', trigger: 'blur' },
        { min: 6, max: 12, message: '请输入3-11位密码', trigger: 'blur' }
      ]
    });
      ...
      formEl.validate((valid, fields) => {
        if (valid) {
          loginByJson({
            ...
            if (resp.meta.code !== '10006') {
              // 登录失败

              ElMessage.error(resp.meta.msg + '，请输入正确的用户名或密码');
              return;
            }
            // 登录成功
          ...
          ElMessage.warning('请核对用户名和密码是否填写');
        }
      });
    };
